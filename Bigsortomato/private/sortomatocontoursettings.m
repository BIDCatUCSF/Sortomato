function hContourSettings = sortomatocontoursettings(~, ~, hSortomatoGraph, axesGraph)
    % SORTOMATOCONTOURSETTINGS Change Sortomato contour plot settings
    ...
    hContourSettings = getappdata(hSortomatoGraph, 'hContourSettings');
    if ~isempty(hContourSettings)
        figure(hContourSettings)
        return
    end % if
    ...
    setappdata(hSortomatoGraph, 'hContourSettings', hContourSettings)
end % sortomatocontoursettings


function sliderlevelscallback(sliderContour, ~, axesGraph, popupColorMap, bColor)
    ...
    [~, hContour] = contourf(axesGraph, xyLocs{2}, xyLocs{1}, xyHistSmooth, ...
...
function slidersmoothingcallback(sliderSmoothing, ~, axesGraph)
...
function popupcolormapcallback(popupColorMap, ~, axesGraph, bColor)
...
function closerequestfcn(hObject, ~, hSortomatoGraph)
    ...
    rmappdata(hSortomatoGraph, 'hContourSettings')
    delete(hObject);
end % closerequestfcn