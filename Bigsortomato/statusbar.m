...
% $Revision: 1.5 $  $Date: 2011/10/14 04:10:04 $
    ...
    rootHandles = [];
    %% adam changed handles to handles.Number since for r2014b and on, handlews are no longer doubles, they are objects
    %% maybe this will work
    numhandles = handles.Number;
    
    if any(numhandles)  % non-0, i.e. non-desktop
        try
            rootHandles = ancestor(numhandles,'figure');
    ...
    if any(numhandles==0), rootHandles(end+1)=0; end
