function sortomatographpushregionsort(hObject, ~, hSortomatoGraph, hSortomatoBase, varargin)
    % SORTOMATOGRAPHPUSHREGIONSORT Sort objects in the selected region
    ...
    hStatus = statusbar(hSortomatoGraph, ['Sorting ' rgnSortString]);
    
    %% Determine whether we are sorting whole tracks or singlets.
    graphType = regexp(get(hSortomatoGraph, 'Name'), ...
    ...
    switch graphType
        ...
        % build a list of objects that belong to the parents, then sort.
            ...
            if xFactory.IsSpots(xObject) % Spot sorting
                ...
                [~, inSpotsIdx] = ismember(inEdges(:), inIdxs);
                ...
                % Map the entries in the selection edge array to new spots.
                [~, inSurfacesIdx] = ismember(inEdges(:), inIdxs);
