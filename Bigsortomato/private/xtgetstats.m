function statStruct = xtgetstats(xImarisApp, xObject, varargin)
    ...
    statLabelBaseNames(isIntensityStatExpanded) = regexprep(...
        statLabelList(isIntensityStatExpanded), ' - Channel \d', '');
    ...
    switch lower(xtgetstatsParser.Results.StatType)
        ...
        case 'id' % single objects or tracks
            % adam added conditional for weird transpose of vectors
            % 2018.01.12
            if isrow(singleStatIdxs)
                    singleStatIdxs = singleStatIdxs';
                    trackStatIdxs = trackStatIdxs';
            end
